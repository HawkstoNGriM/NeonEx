...
#or a corrupt one
wget https://cve.mitre.org/data/downloads/allitems.csv -O allCVEs.csv

#Windows : Invoke-WebRequest -Uri "https://cve.mitre.org/data/downloads/allitems.csv" -OutFile "allCVEs.csv"
#i didnt test this windows command but it should work fine (the only thing that might be odd is the Outfile directory
#Like I'd go: ./allCVEs.csv but i think this should work fine if the script /command is ran from the folder
